...
\begin{document}
	...
	\begin{definition}
		...
		The first map is simply the decomposition map $\oplus_{\alpha} \rho_{U_\alpha X}$,and the second map is the 2-cocycle map
		$$\oplus_{\alpha,\beta}(\rho_{U_\alpha\cap U_\beta,U_\alpha}-\rho_{U_\alpha\cap U_\beta,U_\beta})$$
	\end{definition}
	\begin{example}
		Smooth function sheaf on $C^\infty$ manifolds.$F(U)$ is simply all the smooth functions on $U$ equipped with addition.The restriction map is simply the restriction map of functions.
	\end{example}
\end{document}